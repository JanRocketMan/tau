...
optional `search` tool that is enabled when you configure a web-search provider
API key.
...
Searches the public web. The backend is provider-agnostic and defaults to the
[Parallel Search API](https://docs.parallel.ai/api-reference/search/search) in
Fast mode (high quality within a one-second latency budget); the
[Brave Search API](https://brave.com/search/api/) is supported as a migration
alternative. This tool is **disabled by default**; Tau registers it only when
the selected provider's API key is set in the environment at startup. See
...
Privacy: the query text is sent to the configured search provider, and results
enter the session as untrusted external content. The API key stays in the
process environment: it is never a tool argument, never written to session
history, and is redacted from error output.
...
- **`search`** — current facts, external docs, error messages (when configured).
