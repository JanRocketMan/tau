...
def test_startup_update_notice_reports_newer_stable_release(tmp_path: Path) -> None:
...
def test_startup_update_notice_is_quiet_when_current(tmp_path: Path) -> None:
    notice = startup_update_notice(
        "0.2.0",
...
def test_startup_update_notice_uses_fresh_cache(tmp_path: Path) -> None:
    cache_path = tmp_path / "update-check.json"
...
def test_startup_update_notice_uses_fresh_empty_cache(tmp_path: Path) -> None:
    cache_path = tmp_path / "update-check.json"
    cache_path.write_text(
...
def test_startup_update_notice_refreshes_stale_cache(tmp_path: Path) -> None:
    cache_path = tmp_path / "update-check.json"
    cache_path.write_text(
        '{"checked_at":"2026-01-01T00:00:00+00:00","latest_version":"0.2.0"}\n',
        encoding="utf-8",
...
def test_startup_update_notice_ignores_failures(tmp_path: Path) -> None:
...
def test_startup_update_notice_can_be_disabled(tmp_path: Path) -> None:
    notice = startup_update_notice(
        "0.1.0",
        fetcher=lambda _url, _timeout: (_ for _ in ()).throw(AssertionError("no fetch")),
        cache_path=tmp_path / "update-check.json",
...
def test_startup_update_notice_skips_ci(tmp_path: Path) -> None:
    notice = startup_update_notice(
        "0.1.0",
        fetcher=lambda _url, _timeout: (_ for _ in ()).throw(AssertionError("no fetch")),
        cache_path=tmp_path / "update-check.json",
