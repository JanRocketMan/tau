...
5. **Do not guess thinking support.** Only add `thinking_parameter`, `reasoning = true`, and the per-model `thinking_levels` / `thinking_default` pair when the provider/model accepts those request fields. List only levels the model accepts; the level name is sent verbatim as the parameter value.
